...
.. code:: bash
   ...
   git clone https://github.com/DCMLab/cpe_bach_keyboard.git
...
* `Carl Philipp Emanuel Bach – Works for Keyboard <https://github.com/DCMLab/cpe_bach_keyboard/archive/refs/heads/main.zip>`__


Data Formats
============

Each piece in this corpus is represented by four files with identical names, each in its own folder. For example, the first movement of the *Sonata in F Major*, Wq.50/1, has the following files:

-  ``MS3/wq50n01a.mscx``: Uncompressed MuseScore file including the music and annotation labels.
-  ``notes/wq50n01a.tsv``: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
-  ``measures/wq50n01a.tsv``: A table with relevant information about the measures in the score.
-  ``harmonies/wq50n01a.tsv``: A list of the included harmony labels (including cadences and phrases) with their positions in the score.
...
.. code:: python
   ...
   labels = ms3.load_tsv('harmonies/wq50n01a.tsv')
   notes = ms3.load_tsv('notes/wq50n01a.tsv')
...
For questions, remarks etc., please `create an issue <https://github.com/DCMLab/cpe_bach_keyboard/issues>`__ and feel free to fork and submit pull requests.
